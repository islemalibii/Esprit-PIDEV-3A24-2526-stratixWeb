...
class TacheManager
{
    ...
    private function toMidnight(\DateTimeInterface $date): \DateTime
    {
    ...
    public function isEnRetard(Tache $tache): bool
    {
        ...
        return $this->toMidnight($deadline) < $today;
    }
    ...
    public function getJoursRetard(Tache $tache): int
    {
        ...
        $deadlineNormalized = $this->toMidnight($deadline);

        $today = new \DateTime();
        $today->setTime(0, 0, 0);

        if ($deadlineNormalized >= $today) {
            return 0;
        }

        return (int) $deadlineNormalized->diff($today)->days;
    }
    ...
    public function getJoursRestants(Tache $tache): ?int
    {
        ...
        return (int) $today->diff($deadlineNormalized)->days;
    }
    ...
    // FIXED: Reordered conditions to avoid PHPStan warning
    public function getMatricePriorite(Tache $tache): string
    {
        ...
        if ($estImportant) {
            return 'NON_URGENT_IMPORTANT';
        }
        if ($estUrgent) {
            return 'URGENT_NON_IMPORTANT';
        }
